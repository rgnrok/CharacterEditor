...
namespace CharacterEditor
{
    public class GameSaveManager
    {
        ...
        public bool HasSave(string fileName) => 
            File.Exists(SavePath + fileName + "/FileName.dat");

        public GameSaveManager(ILoaderService loaderService, ICoroutineRunner coroutineRunner)
        {
        ...
        {
            var configData = AllServices.Container.Single<ConfigManager>().ConfigData;

            if (GameManager.Instance != null)
                SaveGame(saveName);
            else
                CreateGame(saveName, configData);
        }

        private void CreateGame(string saveName, CharacterGameObjectData configData)
        {
            if (TextureManager.Instance == null) return;
            var saveFilePath = SavePath + saveName;

            if (!Directory.Exists(saveFilePath))
                Directory.CreateDirectory(saveFilePath);

            var skinMergedTexture = TextureManager.Instance.CharacterTexture;
            var portrait = TextureManager.Instance.CharacterPortrait;
            var characher = Character.Create(Guid.NewGuid().ToString(), configData, skinMergedTexture, null, portrait);
            var characherData = new CharacterSaveData(characher);
            characherData.configGuid = configData.Config.guid;

            File.WriteAllBytes(string.Format("{0}/{1}_Character_texture.png", saveFilePath, characherData.guid), skinMergedTexture.EncodeToPNG());

            if (MeshManager.Instance != null)
            {
                foreach (var mesh in MeshManager.Instance.SelectedSkinMeshes)
                    characherData.faceMeshItems[mesh.MeshType] = mesh.MeshPath;

                File.WriteAllBytes(string.Format("{0}/{1}_Character_face_texture.png", saveFilePath, characherData.guid), MeshManager.Instance.FaceTexture.EncodeToPNG());
            }

            var bf = new BinaryFormatter();
            var file = File.Open(saveFilePath + "/FileName.dat", FileMode.Create);
            var saveData = new SaveData(saveName, characherData);
        ...
        private void SaveGame(string saveName)
        {
            ...
            {
                File.WriteAllBytes(string.Format("{0}/{1}_Character_texture.png", saveFilePath, character.guid), character.Texture.EncodeToPNG());
                File.WriteAllBytes(string.Format("{0}/{1}_Character_face_texture.png", saveFilePath, character.guid), character.FaceMeshTexture.EncodeToPNG());
       ...
       */
        ...
        {
            var saveFilePath = SavePath + saveName;
            if (!File.Exists(saveFilePath + "/FileName.dat"))
                return;

            Debug.Log("Load file " + saveFilePath + "/FileName.dat");

            BinaryFormatter bf = new BinaryFormatter();
            var file = File.Open(saveFilePath + "/FileName.dat", FileMode.Open);

            _saveData = (SaveData)bf.Deserialize(file);
            file.Close();

            loadProcessAction(10);
            await LoadData(loadProcessAction);
        }
        ...
        private async Task LoadCharacters(Action callbackAction)
        {
            ...
            var saveFilePath = SavePath + _saveData.saveName;
            foreach (var characterData in _saveData.characters)
            {
                var textureData = File.ReadAllBytes(string.Format("{0}/{1}_Character_texture.png", saveFilePath, characterData.guid));
                ...
                var faceTextureData = File.ReadAllBytes(string.Format("{0}/{1}_Character_face_texture.png", saveFilePath, characterData.guid));
                ...
                Debug.Log("LoadConfig " + characterData.configGuid);
        ...
        private async Task LoadData(Action<int> loadProcessAction)
        {
        ...
        private IEnumerator LoadPlayerCharacterCallbackCoroutine(PlayerCharacterConfig config, Vector3 position, Action<Character> callback)
        {
            ...
            var character = Character.Create(config.guid, goData, skinTexture, faceTexture, portraitIcon);
        ...
        private IEnumerator LoadConfigCallback(CharacterSaveData characterData, CharacterConfig config, Texture2D cTexture, Texture2D fTexture, Action<Character> loadCharacterCallback)
        {
            ...
            var character = Character.Create(characterData, gameobjectData, cTexture, fTexture, portraitIcon);
            var itemGuids = new List<string>();
            // Надетые айтемы
            foreach (var pair in characterData.equipItems)
                itemGuids.Add(pair.Value.dataGuid);
            // Айтемы в инвентаре
            foreach (var item in characterData.inventoryCeils.Values)
            ...
            _loaderService.ItemLoader.LoadData(itemGuids, (Dictionary<string, ItemData> items) =>
            {
                ...
                //todo only equipItems
                foreach (var ceilPair in characterData.inventoryCeils)
                {
