Shader "CharacterEditor/SkinAndClothRenderShader"
{
	Properties
	{
		...
		_RobeLongTex("Texture RobeLong", 2D) = "white" {}
		_RobeShortTex("Texture RobeShort", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			struct v2f
			{
				...
				float2 uv_13 : TEXCOORD13;
				float2 uv_14 : TEXCOORD14;
			...
			sampler2D _GloveTex;
			sampler2D _ShoeTex;
			sampler2D _RobeLongTex;
			sampler2D _RobeShortTex;
			sampler2D _BeltTex;
			sampler2D _HeadTex;
			...
			float4 _GloveTex_ST;
			float4 _ShoeTex_ST;
			float4 _RobeLongTex_ST;
			float4 _RobeShortTex_ST;
			float4 _BeltTex_ST;
			float4 _HeadTex_ST;
			...
			v2f vert (appdata v)
			{
				...
				o.uv_9 = v.uv.xy * _GloveTex_ST.xy - _GloveTex_ST.zw;
				o.uv_10 = v.uv.xy * _ShoeTex_ST.xy - _ShoeTex_ST.zw;
				o.uv_11 = v.uv.xy * _RobeLongTex_ST.xy - _RobeLongTex_ST.zw;
				o.uv_12 = v.uv.xy * _RobeShortTex_ST.xy - _RobeShortTex_ST.zw;
				o.uv_13 = v.uv.xy * _BeltTex_ST.xy - _BeltTex_ST.zw;
				o.uv_14 = v.uv.xy * _HeadTex_ST.xy - _HeadTex_ST.zw;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 gloveT = tex2D(_GloveTex, i.uv_9);
				fixed4 shoeT = tex2D(_ShoeTex, i.uv_10);
				fixed4 lRobeT = tex2D(_RobeLongTex, i.uv_11);
				fixed4 sRobeT = tex2D(_RobeShortTex, i.uv_12);
				fixed4 beltT = tex2D(_BeltTex, i.uv_13);
				fixed4 headT = tex2D(_HeadTex, i.uv_14);
